...
import { checkPasswordMatch, CreateSessionToken, CreateUser, FindUserByEmail } from '../../services/user/index';

const updateLimit: RequestHandler = async (req, res) => {
  try {
    const { limit } = req.body as { limit: number };

    await prisma.user.update({ where: { id: req.user.id }, data: { limit: limit } });
